...
CHANGES=$(git -c core.quotepath=false log --since="$SINCE" --pretty=format:"" --name-status -- 'inbox-maboyang/' 2>/dev/null \
  | grep -vE '^\s*$' | sort -u || true)

if [ -z "$CHANGES" ]; then
  echo "has_changes=false" >> "${GITHUB_OUTPUT:-/dev/null}"
  echo "本周 inbox-maboyang/ 无变化, 不开 issue。"
  exit 0
fi

# 三档 bucket
highlight=$(echo "$CHANGES" | grep -E "inbox-maboyang/08-reports-and-feedback/" || true)
content=$(echo "$CHANGES" | grep -E "inbox-maboyang/(01-keyword-research|03-content-briefs|04-production)/" || true)
other=$(echo "$CHANGES" | grep -vE "inbox-maboyang/(08-reports-and-feedback|01-keyword-research|03-content-briefs|04-production)/" || true)
