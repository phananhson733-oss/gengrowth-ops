...
此工具的唯一源目录是 `tools/internal/backlink-opportunity-tool/`。`gengrowth-ops/tools/` 是 Wiki 的自动镜像，不能作为编辑或运行源。

```bash
cd /Users/awayer_mini/gengrowth-wiki/tools/internal/backlink-opportunity-tool
cp .env.example .env
chmod 600 .env
# 编辑 .env，至少填入 SERPAPI_API_KEY 或 AHREFS_API_KEY
chmod +x run-server.sh install-launchd.sh uninstall-launchd.sh
bash install-launchd.sh
```

服务由 `com.gengrowth.backlink-opportunity-tool` 以 `launchd` 常驻运行，仅监听 `127.0.0.1:4318`。检查运行状态：

```bash
curl -fsS http://127.0.0.1:4318/api/health
launchctl print gui/$(id -u)/com.gengrowth.backlink-opportunity-tool
```

日志位于 `~/Library/Logs/backlink-opportunity-tool/`。卸载服务（不会删除工具数据或日志）：

```bash
bash uninstall-launchd.sh
```
