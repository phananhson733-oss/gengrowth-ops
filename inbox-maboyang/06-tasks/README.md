...
> `inbox-maboyang/` 不在 sync 镜像里，是 Ops 专属可写工作台 —— 任务在这里编辑永远不会被覆盖、不会和 wiki sync 撞车。

## 目录结构

```text
inbox-maboyang/06-tasks/
├── README.md
...
1. 每个任务一个独立文档，放在 `inbox-maboyang/06-tasks/tasks/`。
2. 文件命名 `YYYY-MM-DD-<topic>-task.md`。
3. 单任务文档内保留 1 条主任务行（`- [ ] ... #task`）和验收标准。
4. 统一在 `inbox-maboyang/06-tasks/task-board.md` 查看聚合看板。
