...
| **S** | **`Ref_URLs`** | 人工，建卡时填 | 该词当前 SERP Top 3 的竞品页面 URL，用 `\|` 分隔。STEP 2 搜证前提前存入，避免重复查找。T3 可留空。 |
...
2. **1+N 折叠升级（同 Intent + 同 Query Job）**：
   ...
   * **折叠前三步判断（按序执行，任一否决即拆开写）**：
     1. **SERP 重合度**：Google 分别搜两个词，Top 10 里相同 URL ≥ 6 个 → 可折叠；< 4 个 → 拆开。30 秒完成，最靠谱的折叠依据。
     2. **意图粒度**：两个词搜索场景是否完全相同？场景不同（如"猫的年龄"vs"公寓空间"）即使大意图相同也拆开，同场景不同措辞才合并。
     3. **站点内容量**：当前站点总文章 < 50 篇时，SERP 重合度中等（4-5 个相同 URL）也允许折叠，优先堆厚度；> 200 篇后趋向精细拆分，每篇只聚焦一个小主题。
