...
| 规则项 | 要求 |
|---|---|
| 数量 | 每篇 ≥3 条工具页内链（前20% 一条 + 中段一条 + 结尾一条）|
| 位置 | **正文前 20%（首屏内）必须有第一条**——此位置权重最高；中段一条；结尾一条。不要全堆在末尾 |
| 锚文本 | 功能描述词为主，三条锚文本表述不得完全相同；具体比例分布见**补丁九** |
...
> 本补丁与**补丁三**配合使用：补丁三规定了工具页内链的数量和位置，本补丁规定三条内链的锚文本应如何分配类型（精确 / 部分匹配 / 自然描述），两者都需要执行。
...
| 链接位置 | 性质 | 动作 |
|---|---|---|
| 导航栏 / 侧边栏 | 通常 nofollow 或全局重复链接，不传 PageRank | 确保这些链接指向的页面在主题上正确——即使不传权重，语义方向仍被 Google 读取；不要把导航链接指向低价值/无关页面 |
| CTA 组件（React 客户端渲染）| JS 渲染，PageRank 传递效率低，语义信号仍传递 | 不能用组件链接替代正文静态链接；两者必须同时存在——正文静态链接传权重，组件传语义（Req 2 正在补全静态链接，此为背景依据）|
| 正文 HTML 静态链接 | dofollow，传 PageRank + 语义信号 | 这是权重传递的主通道，优先保证正文静态链接到位，再叠加组件链接 |
...
**适用场景：** 每季度内链审计时，处理「无任何入链」的孤岛页面。

孤岛页面只能靠 sitemap 被爬取，没有内链传递权重和语义信号，排名普遍很差。修复时有几个容易踩的坑：

**修复优先级排序（从高到低）：**

1. 有稳定搜索量的关键词对应页面（修复后排名提升最快）
2. 有外链但无内链的页面（外链权重进来后无法在站内流转，浪费最严重）
3. 排名在第 2-4 页、差临门一脚的页面

暂缓修复：内容质量很差的页面（先提升内容，再补内链）、话题过于边缘的页面（性价比低）。

**修复操作四条规则：**

| 规则 | 说明 |
|------|------|
| 从高权重页面加链接 | 从孤岛页面或低权重页面加的内链效果微乎其微，必须从权重较高的老页面里找语境合适的位置 |
| 上下文必须自然 | 找到真正能「接住」目标页面内容的源页面段落，不要强塞「了解更多」 |
| 先检查目标页面质量 | 如果页面内容本身差，补内链是往漏桶加水；先升级内容质量，再补入链 |
| 分批次修复 | 每次集中修复 5-8 个孤岛页面，观察 2-3 周数据再继续；突然大量新增内链会触发 Google 重新评估 |

**AstrologyWiki 当前孤岛风险：** Req 5（内链健康度扫描）尚未上线，无法系统追踪孤岛页面数量。在 Req 5 就绪前，可手动导出 Google Search Console「链接」报告，找出内部链接数为 0 的页面作为优先修复对象。

---

### 补丁十九：锚文本集中注入——主题强化

**适用场景：** 某个核心工具页或 Pillar 页排名卡在第 2-4 页，想用内链主动推上去。

当一个页面的关键词布局已经到位，但排名仍不理想时，跨多篇相关文章统一向该页面集中注入含目标关键词的锚文本，比在目标页面内再堆关键词有效得多——跨页面的语义背书是 Google 的强信号。

**操作步骤：**

1. 确定目标页面和目标关键词（如 `/en/birth-chart-calculator`，目标词 `birth chart calculator`）
2. 找出站内语义相关的 5-8 篇文章（非强关联的文章不要加，会稀释主题信号）
3. 在每篇文章中找最自然的段落，插入指向目标页面的内链，锚文本使用目标词或其变体：
   - 精确词（20-30%）：`birth chart calculator`
   - 部分匹配（40-50%）：`free natal chart tool`、`generate your birth chart`
   - 自然描述（20-30%）：`try our astrology tool`
4. 分 2-3 周内分批完成，不要一天全加上
5. 观察目标页面排名变化，通常 2-4 周可见效

**整站上限：** 同一目标页面的精确匹配锚文本，全站不超过页面总数的 10%（与补丁九一致）。AstrologyWiki 当前约 311 篇，即任意目标页精确锚文本内链上限约 31 条。

**进阶用法：锚文本修正页面定位**

若某页面 Google 把它排在词 B 上，但你想让它排词 A——可以把指向该页面的内链锚文本统一改为词 A 的变体，引导 Google 重新理解该页面的主题归属。这是低成本的页面定位修正手段，不需要改页面内容。

---

### 补丁二十：内链审计工具组合

**适用场景：** Req 5 内链健康度扫描的工具选型，以及季度内链审计的执行方式。

| 工具 | 主要用途 | 适用规模 |
|------|---------|---------|
| **Screaming Frog** | 爬取整站，导出所有内链、断链、noindex 页面；是内链审计的首选工具。免费版支持 500 个 URL | 小型网站首选 |
| **Ahrefs Site Audit** | 可视化内链结构图、识别孤岛页面、追踪内链健康度变化趋势 | 中大型网站持续监控 |
| **Google Search Console** | 「链接」报告：每个页面收到的内部链接数量和来源；免费且最权威，但缺少可视化分析 | 所有网站 |
| **LinkWhisper**（WordPress 插件） | 自动推荐相关内链，快速扩充内链密度；推荐质量参差不齐，需人工审核后再发布 | WordPress 内容站 |

**AstrologyWiki 推荐组合：**
- 日常监控：GSC「链接」报告（免费，每月看一次）
- Req 5 自动化扫描：Vercel Cron Job（已在 Req 5 需求文档中定义）
- 季度深度审计：Screaming Frog 爬取 + Ahrefs 孤岛图（每季度一次）
...
|--------|---------|
| 断链（404 内链） | 任意 1 个触发 |
| 内链指向 noindex 页面 | ≥5 条触发（见补丁十三） |
| 接收内链最多的页面是否为核心目标页 | 如果 Top 20 里出现「关于我们」「联系方式」等非核心页，需重新规划 |
| 精确锚文本集中度 | 任意目标页精确锚文本 > 总页面数 10%，触发优化（见补丁九） |
| 单页链出过多 | 超过 15 条触发（见补丁十一） |
...
| | 十八 | 孤岛页面修复 | 无规定 | 分批次从高权重页面注入链接；优先修复有搜索量/有外链无内链的页面；先提升内容质量再补入链 |
| | 十九 | 锚文本集中注入 | 无规定 | 对核心目标页从 5-8 篇相关文章统一注入含目标词锚文本；分 2-3 周分批；可用于修正页面关键词定位 |
| | 二十 | 内链审计工具组合 | 无规定 | Screaming Frog（爬取）/ Ahrefs（孤岛图）/ GSC（内链报告）；五项核心审计指标 |
