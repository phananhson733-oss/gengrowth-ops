...
implementation_commit: 6b2cdf90
---
...
| 生产部署              | ✅ 已上线    | `main` 已更新至 `6b2cdf90`；Vercel Production 部署 `dpl_H1Tj3TCrfpnPYZe7eVSADwcNCYfP` 状态为 Ready，并已绑定 `www.astrologywiki.com`。 |

**实现位置**：`xdawayer/oracle` → `main` → `6b2cdf90`。

**生产地址**：<https://www.astrologywiki.com>。

**范围与验收说明**：

- 生产域名已验证：首页新版 Title/H1、6 个 FAQ、Organization 联系信息、Haaland Wiki 页面、文章 CTA 与 Birth Chart Calculator 目标页均可正常访问；CTA 点击可从 Wiki 页面进入 `/en/birth-chart-calculator`；`/health` 与 `/api/region` 均返回 200；1366px 与 2048px 桌面导航无重叠、无横向溢出，浏览器控制台无错误。
