...
| 4 | `astrologywiki-social-daily/SKILL.md` | 38,682 B | Markdown 当前执行规则 |
...
│   ├── 06-daily-content-recommendations/     # 内容生产
│   │   ├── README.md
│   │   ├── 当前制作/
│   │   ├── 待发布/
│   │   ├── 已发布/
...
2. 更新 `06-daily-content-recommendations/README.md`：建立待发布、待选择/待确认、已发布待闭环的当前队列，并把候选附件从默认读取降权。
...
- [ ] 在 `06` 下按 `当前制作/待发布/已发布/历史生产记录` 分批移动；每批只移动状态已确认的文件。
...
- 从 `04-production/README.md` 最多两次点击可以到达当前制作、待发布、当前周报和生产 SOP。
