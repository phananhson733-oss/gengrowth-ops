...
### Claude Code
- Python scripts (`scripts/`) can be run via Bash tool: `python3 scripts/calculate_metrics.py`
- Data input: user pastes JSON or provides file path; read with Read tool
- No external dependencies (stdlib only)

### Codex
- Python scripts (`scripts/`) can be run via shell: `python3 scripts/calculate_metrics.py`
- Data input: user pastes JSON or provides file path
- Browser can fetch public metrics from social platforms when user is logged in

### Shared conventions
- Scripts are pure computation (no network, no file writes beyond stdout)
- All benchmarks in `references/platform-benchmarks.md` are upstream defaults requiring verification
- Edits to this file propagate to both platforms immediately via symlink

### AstrologyWiki Data Source Notes
When analyzing AstrologyWiki TikTok accounts, fetch data using `getAllData` (JSON format) — NOT `getData` (CSV format has a truncation bug).

**Primary data fetch command (returns ALL sheets as JSON):**
```
curl -sL "https://script.google.com/macros/s/AKfycbyKsZCN5G8Ik-9bbh26GHPxfPflusxUy-13hNy9h-sb3qVdqf7KUoJZYvPTApapbKFS/exec?action=getAllData"
```
Response is JSON: `{"accounts_latest": "csv_string", "posts_latest": "csv_string", "post_history": "csv_string", ...}`

Each value is a CSV string. Parse it with a CSV parser (handle commas in quoted fields).

**Why not use `getData&sheet=posts_latest`:**
The CSV endpoint truncates each row to 200 bytes due to a character encoding issue with emoji/Unicode in captions. The `getAllData` JSON endpoint returns the same data WITHOUT truncation. This is a confirmed bug as of 2026-07-23.

**Available data in posts_latest (via getAllData):**
- post_id, username, post_url, content_type, published_at, caption
- first_seen_at, last_seen_at, snapshot_date, captured_at
- **views, likes, comments, favorites, shares** ← these ARE present when fetched via getAllData
- collection_status, missing_fields

**Accounts tracked:** astrologywiki, filestarsx, miraaastrology, shirley527146

**Fallback if getAllData is too large:** use `getData&sheet=post_history` (this sheet has shorter captions and does not trigger the truncation bug). But note `post_history` may have fewer posts (only those captured in snapshot runs).

**Account overview (works fine with either method):**
```
...
**Data limitations to always disclose:**
- Only public metrics available (views, likes, comments, favorites, shares)
- No completion rate, average watch time, profile visits, or link clicks
- Snapshot frequency depends on when the capture script last ran (check `snapshot_date` column)
- If only one snapshot date exists, trend analysis is not possible — state this clearly
