...
This skill is installed as a single canonical source shared by both Claude Code and Codex via symlinks.

### Claude Code
- Social Listening research: use web search tools or browser-harness skill when available
- File references: relative paths (`references/platforms.md`) resolve from the skill directory

### Codex
- Social Listening research: use browser/Chrome inspection for logged-in platforms
- Scripts: not included for this skill (strategy-only; no executable automation)
- File references: same relative paths resolve identically

### Shared conventions
- Never auto-publish; always output drafts for human review
- `product_context` and `account_context` must be provided at runtime
- This file is the single source of truth; edits here propagate to both platforms immediately
