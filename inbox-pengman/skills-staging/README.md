...
> **Canonical source（单一源）：** `~/gengrowth-ops/inbox-pengman/skills-staging/`
...
本目录包含三个通用海外社媒运营 Skill，可被 GenGrowth 不同产品复用：
...
三个 Skill 保持产品无关。产品差异通过运行时 context 输入解决。
...
当前 canonical source 在 `inbox-pengman/skills-staging/`（受限于写权限）。两个平台已通过 symlink 注册：

```
~/.claude/skills/gengrowth-social → ~/gengrowth-ops/inbox-pengman/skills-staging/gengrowth-social
~/.claude/skills/gengrowth-tiktok-strategist → ~/gengrowth-ops/inbox-pengman/skills-staging/gengrowth-tiktok-strategist
~/.claude/skills/gengrowth-social-media-analyzer → ~/gengrowth-ops/inbox-pengman/skills-staging/gengrowth-social-media-analyzer
...
1. CEO 审批
2. `mv skills-staging/gengrowth-* ~/gengrowth-ops/tools/internal/skills/`
3. 更新 symlinks：
   ```bash
