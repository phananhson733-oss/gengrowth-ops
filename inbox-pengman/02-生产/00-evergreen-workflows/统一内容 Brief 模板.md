...
| `product` | 使用稳定产品 ID；当前为 `astrologywiki`。未来新增产品时新增属性值，不复制生产目录 |
| `account` | 使用稳定账号 ID；当前为 `astrologywiki / miraaastrology`，未匹配时为 `unassigned`。不得添加序号、平台或展示说明 |
| `account_handle` | 平台可见 Handle，例如 `@astrologywiki`；未匹配账号时留空 |
...
| `scheduled_timezone` | `scheduled_at` 没有自带偏移时的解释时区；当前默认 `America/Chicago` |
