---
content_id: aw-scorpio-space-or-done-dualhost-02
title: "Is Scorpio taking space — or is the relationship over?"
...
platform: tiktok
pool: Evergreen
pillar: Scorpio relationship / silence / communication boundary
...
- **目标用户**：处在约会早期、争吵后或 no-contact 阶段的美国 18–35 岁用户；Scorpio 本人与正在等待回复的一方。
- **内容目标**：测试双人问答能否承接“需要空间还是关系结束”的高冲突问题，同时不美化 ghosting。
- **用户问题**：Scorpio 不回复时，是在冷静，还是关系已经结束？
...
- **约束和禁止声明**：不得将 ghosting 正当化；不使用 `avoidant` 等临床标签；不得以性别写角色。
- **制作或实验说明**：与 `aw-scorpio-friends-to-lovers-dualhost-01` 同属双人口播测试，但必须错开场景、Hook 和第二角色的问题；固定 Miraa 主播、字幕和背景，新增角色只承担提问。
