...
test("account and capture reads require one of four Hermes Feishu identities", () => {
  ...
  for (const profile of ["default", "social", "pm", "ops"]) {
    assert.deepEqual(resolveInvocationIdentity(command, {
      HERMES_SESSION_PLATFORM: "feishu", HERMES_SESSION_PROFILE: profile,
      HERMES_SESSION_USER_ID: "ou_reader", HERMES_SESSION_CHAT_ID: `oc_${profile}`,
    }), { mode: "social", actorId: "ou_reader", chatId: `oc_${profile}`, profile });
  }
  for (const profile of ["", "ceo", "code", "hr", "unknown"]) {
    assert.throws(() => resolveInvocationIdentity(command, {
      HERMES_SESSION_PLATFORM: "feishu", HERMES_SESSION_PROFILE: profile,
      HERMES_SESSION_USER_ID: "ou_reader", HERMES_SESSION_CHAT_ID: "oc_rejected",
...
test("Hermes provenance binds each allowed profile to its exact gateway anchor", () => {
  const argv = ["pool", "list", "--config", SOCIAL_RUNTIME_CONFIG_PATH];
  const command = parseCommand(argv);
  const runner = path.resolve(new URL("../shortdrama_ctl.mjs", import.meta.url).pathname);
  const sessionId = "a1b2c3d4e5f6";
  ...
  const rowsFor = (profile) => {
    const named = profile !== "default";
    const home = named
      ? `/Users/awayer_mini/.hermes/profiles/${profile}`
      : "/Users/awayer_mini/.hermes";
    const profileArgs = named ? ` --profile ${profile}` : "";
    const gateway = `${python} -m hermes_cli.main${profileArgs} gateway run --replace --external-supervisor`;
    const supervisor = `${python} -m hermes_cli.stderr_timestamp --error-log ${home}/logs/gateway.error.log -- ${gateway}`;
    const wrapped = [
      `source ${snapshot} >/dev/null 2>&1 || true`,
      `builtin cd -- ${home} || exit 126`,
      `eval '${direct}'`,
      "__hermes_ec=$?",
      "umask 077",
      `{ export -p > ${snapshot}.tmp.$BASHPID && mv -f ${snapshot}.tmp.$BASHPID ${snapshot}; } 2>/dev/null || rm -f ${snapshot}.tmp.$BASHPID 2>/dev/null || true`,
  ...
  const inspect = (profile, rows) => inspectTrustedSocialInvoker({
    argv, command, configPath: SOCIAL_RUNTIME_CONFIG_PATH, profile, pid: 100,
    runnerPath: runner, nodePath: process.execPath, readProcess: (pid) => rows.get(pid),
  });

  for (const profile of ["default", "social", "pm", "ops"]) {
    assert.equal(inspect(profile, rowsFor(profile)), true, profile);
  }
  assert.equal(inspect("pm", rowsFor("social")), false);
  assert.equal(inspect("ceo", rowsFor("social")), false);
});

test("Hermes provenance accepts the official atomic snapshot wrapper exactly", () => {
  const profile = "ops";
  const argv = ["pool", "list", "--config", SOCIAL_RUNTIME_CONFIG_PATH];
  const command = parseCommand(argv);
  const runner = path.resolve(new URL("../shortdrama_ctl.mjs", import.meta.url).pathname);
  ...
  const tempRoot = "/var/folders/c8/k7q0dcp13rd8590xbtxs_9n80000gn/T";
  const snapshot = `${tempRoot}/hermes-snap-${sessionId}.sh`;
  const home = "/Users/awayer_mini/.hermes/profiles/ops";
  const python = "/Users/awayer_mini/.hermes/versions/hermes-official-9ed06ca2/.venv/bin/python";
  const direct = `/usr/bin/env node ${runner} ${argv.join(" ")}`;
  const gateway = `${python} -m hermes_cli.main --profile ops gateway run --replace --external-supervisor`;
  const supervisor = `${python} -m hermes_cli.stderr_timestamp --error-log ${home}/logs/gateway.error.log -- ${gateway}`;
  const wrapped = [
    `source ${snapshot} >/dev/null 2>&1 || true`,
    ...
    "__hermes_ec=$?",
    "umask 077",
    `__hermes_snap_tmp=$(mktemp ${snapshot}.tmp.XXXXXXXXXX) && { { ( unset \${!HERMES_SESSION_*} \${!HERMES_CRON_AUTO_DELIVER_*} AI_AGENT HERMES_AGENT HERMES_UI_SESSION_ID 2>/dev/null; export -p; ) || true; } > "$__hermes_snap_tmp" && mv -f "$__hermes_snap_tmp" ${snapshot}; } 2>/dev/null || rm -f "$__hermes_snap_tmp" 2>/dev/null || true`,
    `printf '\\n__HERMES_CWD_${sessionId}__%s__HERMES_CWD_${sessionId}__\\n' "$(pwd -P)"`,
    "exit $__hermes_ec",
  ].join("\n");
  const rows = new Map([
  ...
  for (const replacement of [
    ...
    ["unset ${!HERMES_SESSION_*}", "unset HERMES_SESSION_PROFILE"],
    ["exit $__hermes_ec", "whoami\nexit $__hermes_ec"],
  ]) {
    const changed = new Map(rows);
    changed.set(90, { ...rows.get(90), args: rows.get(90).args.replace(...replacement) });
    assert.equal(inspect(changed), false, replacement[0]);
  }
});
