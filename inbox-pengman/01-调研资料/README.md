---
title: 调研资料入口
project: astrologywiki
type: workspace-index
status: active
owner: Pengman
updated: 2026-08-07
---

# 调研资料入口

这里保存平台策略、方法论、工具研究和历史证据；不承担当前内容状态、最终稿或发布数据。日常生产从 [[inbox-pengman/02-生产/README]] 开始，只有需要验证依据、重新评估策略或比较工具时才进入本目录。

## 分类入口

| 目录 | 内容 | AI 默认读取 |
|---|---|---:|
| [[inbox-pengman/01-调研资料/平台与策略/README.md]] | 当前仍有参考价值的平台和内容方向研究 | 否 |
| [[inbox-pengman/01-调研资料/方法论/README.md]] | 通用运营框架和方法论底稿 | 否 |
| [[inbox-pengman/01-调研资料/工具调研/README.md]] | 视频、视觉、文本、搜索和社媒工具研究 | 否 |
| [[inbox-pengman/01-调研资料/候选与热点研究/README.md]] | 仅在权限门允许时保存周一候选、Hot、补库或明确重排信息来源 | 否 |
| [[inbox-pengman/01-调研资料/历史调研/README.md]] | 首轮方案、平台样本和早期思考 | 否 |
| [[inbox-pengman/01-调研资料/历史流程/README.md]] | 已被当前 README / Skill / SOP 取代的旧总流程和鱼骨图 | 否 |

## 事实来源边界

- 当前内容状态和最终稿：单条主生产记录。
- 发布数据和 `decision / next_test`：`02-生产/03-data-review/` 的对应周报或专项复盘。
- 当前公共表达和候选研究规则：`skills/astrologywiki-social-workflow/SKILL.md`。
- 竞品账号与视频数据：在线 Google Sheet；本地旧快照只作历史证据。
- 本目录文件可以解释“为什么曾经这样判断”，不能覆盖当前生产入口中的明确规则。

## 存放规则

- 新调研先确认能否补充现有文件，不建立平行总览。
- 研究完成后标明日期、证据范围、当前有效性和被哪个生产规则采用。
- 可重新生成的缓存、模型输出和媒体文件不放入 Git；只保留来源链接、生成说明和必要参数。
