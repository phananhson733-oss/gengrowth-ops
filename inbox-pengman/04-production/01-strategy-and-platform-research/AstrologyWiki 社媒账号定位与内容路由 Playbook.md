---
title: AstrologyWiki 社媒账号定位与内容路由 Playbook
...
# AstrologyWiki 社媒账号定位与内容路由 Playbook

> 本文档回答“这条内容应该由哪个当前账号承接”。当前只启用 `@astrologywiki` 与 `@miraaastrology`；它不是固定账号数量承诺，也不要求两个账号每天都发。未来增加、恢复或停用账号时，先通过本文的账号启用门，再由当前周计划决定当周配额。

本文只维护账号角色、内容边界、形式和路由规则。周度产能、内容池、Batch、排期与 `content_stage` 仍以 [[inbox-pengman/04-production/00-evergreen-workflows/weekly-rolling-content-production-sop]]、当前周计划和单条主生产记录为准。
...
| 未来可能新增的账号 | **not activated** | 尚无当前职责 | 不进入周计划、候选路由、生产配额或自动化发布范围 |
...
4. 账号只有在当前周计划明确列为 active 后，才进入候选路由和生产队列。
...
- 现行 SOP、Skill、模板和接手入口应统一引用本文的新文件名和当前 active 账号口径。
